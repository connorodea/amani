...
final class OverlayWindowController {
    ...
    init(searchController: SearchController? = nil) {
        // A default-parameter-value expression isn't actor-isolated even on a @MainActor type,
        // so the SearchController() default is constructed inside the (isolated) init body
        // instead of as a parameter default — same external behavior, `OverlayWindowController()`
        // still works with no arguments.
...
- [ ] **Step 4: No SearchView/OverlayWindow rewiring needed**

Unlike an earlier draft of this plan, Task 12 already shipped `SearchView` computing orb state
internally (`OrbView(state: searchController.query.isEmpty ? .idle : .active)`) and
`OverlayWindowController` calling `SearchView(searchController:onSubmit:)` with no `orbState:`
argument — there was never a fixed `orbState: OrbState` parameter to remove. Task 13 only
replaces the placeholder `Sources/Amani/UI/OrbView.swift` (Step 3 above) with the real SceneKit
implementation; `OrbState`'s two cases (`idle`/`active`) and its call site are already correct
...
```bash
git add Sources/Amani/UI/OrbView.swift Tests/AmaniTests/OrbStateTests.swift
